...
@dfpipe
def mutate_if(df, predicate, fun, prefix='mutated'):
    """
    ...
    for col in df:
        try:
            if predicate(df[col]):
                cols.append(col)
                mutated = f"{prefix}_{col}"
                df[mutated] = df[col].apply(fun)
        except:
            pass
...
@dfpipe
def transmute_if(df, predicate, fun, prefix='transmuted', *keep_columns):
    """
    Creates columns and then returns those new columns and optionally specified
    original columns from the DataFrame.
    ...
    for col in df:
        try:
            if predicate(df[col]):
                transmuted = f"{prefix}_{col}"
                df[transmuted] = df[col].apply(fun)
                transmuted_cols.append(transmuted)
        except:
            pass

    columns = transmuted_cols + list(keep_cols)
    return df[columns]
